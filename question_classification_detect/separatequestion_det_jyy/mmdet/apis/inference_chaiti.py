    ...
    for _i in range(num):
        ...
        for _j in range(_i + 1, num):   #对剩余的而进行遍历
            ...
            if(np_classes[i] == np_classes[j]):
                ...
                if((iou >= 0.85 or small_iou > 0.85) and (scores[i] < 0.96 or scores[j] < 0.96)):  #对大于设定阈值的检测框进行滤除
                    ...
                    if(scores[i] > 0.99):
                        ...
                        keep_index.remove(i)
                        suppressed[i] = 1
                    elif(scores[i] > 0.94 and scores[i] - scores[j] > 0.1):
                        suppressed[j] = 1
                    elif(scores[j] > 0.94 and scores[j] - scores[i] > 0.1):
                        keep_index.remove(i)
                        suppressed[i] = 1
...
colors = [_to_color(x, base) for x in range(17)]
...
def draw_result(scale, path, img_name, img, segms_list, labels, bboxes):
    ...
    CLASSES = ('ptext', 'htext', 'pformula', 'hformula', 'ld', 'nld', 'oc', 'mixformula', 'graph', 'excel', 'p_formula_set', 'p_up_down', 'h_formula_set', 'h_up_down', 'p_matrix', 'h_matrix')
    portion = os.path.splitext(img_name)
    o_txt = open(os.path.join("/data/ouyangshizhuang/data/chaiti_testdata/12_5_result_2", portion[0] + ".txt"), "w")
